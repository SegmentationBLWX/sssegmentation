...
class MobileSAMPredictor(SAMPredictor):
    def __init__(self, sam_cfg=None, use_default_sam_t_5m=False, device='cuda'):
        if sam_cfg is None:
            sam_cfg = {
                'backbone': {
                    ...
                    'mbconv_expand_ratio': 4.0, 'local_conv_size': 3, 'pretrained': False, 'pretrained_model_path': ''
                },
                'prompt': {
        ...
        super(MobileSAMPredictor, self).__init__(
            use_default_sam_h=False, use_default_sam_l=False, use_default_sam_b=False, sam_cfg=sam_cfg,
        )
    '''buildsam'''
...
class MobileSAMAutomaticMaskGenerator(SAMAutomaticMaskGenerator):
    def __init__(self, points_per_side=32, points_per_batch=64, pred_iou_thresh=0.88, stability_score_thresh=0.95, stability_score_offset=1.0, device='cuda',
                 ...
                 min_mask_region_area=0, output_mode='binary_mask', sam_cfg=None, use_default_sam_t_5m=False):
        user_defined_sam_predictor = MobileSAMPredictor(sam_cfg=sam_cfg, use_default_sam_t_5m=use_default_sam_t_5m, device=device)
